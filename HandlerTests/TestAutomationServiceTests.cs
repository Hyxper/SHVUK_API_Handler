...
namespace HandlerTests
{
    public class TestAutomationServiceTests
    {
        ...
        //exception throws for GetWOInfo

        [Fact]
        public void GetWOInfo_ThrowsOnJsonProcessingError()
        {
            _mockApiHandler.Setup(a => a.Get(It.IsAny<string>()))
                           .Throws<JsonException>();

            Assert.Throws<SHVUKApiException>(() => _service.GetWOInfo("testSerial"));
        }

        [Fact]
        public void GetWOInfo_ThrowsOnApiEndpointError()
        {
            _mockApiHandler.Setup(a => a.Get(It.IsAny<string>()))
                           .Throws<HttpRequestException>();
        ...
        [Fact]
        public void GetWOInfo_ThrowsOnApiParameterError()
        {
            _mockApiHandler.Setup(a => a.Get(It.IsAny<string>()))
                           .Throws<ApiParameterException>();
        ...
        [Fact]
        public void GetWOInfo_ThrowsOnApiInvocationError()
        {
            _mockApiHandler.Setup(a => a.Get(It.IsAny<string>()))
                           .Throws<ApplicationException>();
        ...
        [Fact]
        public void GetWOInfo_ThrowsOnApiResponseValidationError()
        {
            var fakeDto = new VerifyCurrentRoutingFunction_DTO
            {
                ErrorMessage = "Some Error"
        ...
        [Fact]
        public void VerifyTestStatusAndMove_FallsOver()
        {
            IApiHandler apiHandler = new ApiHandler(new HttpService(HttpClientFactory.CreateJSONClient()));
            ITestAutomationService testAutomationService = new TestAutomationService(apiHandler);
            string serialNumber = "111129818"; //exists in both test and live.
            ...
            Assert.Throws<SHVUKApiException>(() => testAutomationService.SaveTestStatusAndMove(serialNumber, testResult, op, ccn));
        }
    }
}